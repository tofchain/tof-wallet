...
**TOF为合作伙伴定制了对接需要的TOF2.0钱包版本，对接钱包内嵌`TOF-API`模块，模块内封装了TOF-SDK的功能，用HTTP协议访问接口，支持`JSON—RPC`和`Restful`两种格式。**

[主网或测试版钱包下载地址](https://github.com/TOF-io/TOF-v2/releases)

[TOF-API离线操作工具下载地址](http://TOF-cn.oss-cn-hangzhou.aliyuncs.com/2.1/TOF_API-offline_v2.0.0.tar.gz)

## 设置

	`TOF-API`模块默认访问的端口号是18004，可以在TOF.ncf配置文件中做修改，如下：
...
## 说明
	...
	为了更好的理解TOF2.0的相关业务，和接口返回值的含义，提前在这里做一些说明。

### 在线与离线

`TOF-API`模块提供了若干在线接口和离线接口。

在线接口：钱包必须正常运行，且能够连接网络中的其他节点，能够正常同步区块和广播数据。在调用在线接口之前，最好是已经同步到最新区块。接口所产生的数据都会保存在钱包中。例如创建账户、修改密码、转账、获取区块头等。

离线接口：TOF2.0提供了一个专门用于[离线操作的TOF-API工具](http://TOF-cn.oss-cn-hangzhou.aliyuncs.com/2.1/TOF_API-offline_v2.0.0.tar.gz)。无需安装钱包，可独立运行在一台没有连接网络的服务器上。用户通过调用离线接口，传入相关的参数，获取返回值，相应数据不会存入钱包。例如离线创建账户、离线组装转账交易、离线签名等。

### 字段描述

**链的chainId:**

	TOF2.0支持多链并行和跨链转账，每条链通过链ID来区分，TOF主网的链ID为1，TOF测试网的链ID为2。

**链的资产：**

	TOF2.0支持每条链除了默认的资产外，可根据业务需要，动态添加资产。每条链的每种资产通过链ID和资产ID的复合主键来区分。例如TOF主网的TOF，chainId=1,assetId=1

**交易的type值：**

	TOF2.0默认有多种交易，每种交易的功能不同，调用接口查询交易详情时，可通过type字段来区分不同交易类型，以下是交易类型的枚举值：
...
- **`RESTFUL`访问方式**
     ...
     其余请参考 [RESTFUL 接口文档](https://github.com/TOF-io/TOF-v2/blob/master/module/TOF-api/documents/TOF-api_RESTFUL.md)
...
[JSON-RPC 接口文档](https://github.com/TOF-io/TOF-v2/blob/master/module/TOF-api/documents/TOF-api_JSONRPC.md)

[RESTFUL 接口文档](https://github.com/TOF-io/TOF-v2/blob/master/module/TOF-api/documents/TOF-api_RESTFUL.md)

_**附：**_ 官方已提供TOF-SDK-4J工具，有使用JAVA做对接的合作伙伴，可使用工具对接`TOF-API`模块，详见：[TOF-SDK-4J使用说明](https://github.com/TOF-io/TOF-v2-sdk4j/blob/master/README.md)

## 接口调试

我们提供了`Postman`接口调式工具的导入文件(`JSON-RPC`和`RESTFUL`)，导入后，即可调试接口

[JSON-PRC 接口调试-POSTMAN导入文件](https://github.com/TOF-io/TOF-v2/blob/master/module/TOF-api/documents/TOF-api_Postman_JSONRPC.json)

[RESTFUL 接口调试-POSTMAN导入文件](https://github.com/TOF-io/TOF-v2/blob/master/module/TOF-api/documents/TOF-api_Postman_RESTFUL.json)
...
```json
{
  "success" : true,
  "data" : {
    "list" : [ "tTOFeBaMujLBcZWfE2wHKnZo7PGvqvNrt6yWG" ]
  }
}
```
...
_**request path:**_
/api/account/password/tTOFeBaMujLBcZWfE2wHKnZo7PGvqvNrt6yWG
...
_**request path:**_
/api/account/prikey/tTOFeBaMujLBcZWfE2wHKnZo7PGvqvNrt6yWG
...
```json
{
  "success" : true,
  "data" : {
    "value" : "tTOFeBaMujLBcZWfE2wHKnZo7PGvqvNrt6yWG"
  }
}
```
...
```json
{
  "path" : "e:\\tTOFeBaMujLBcZWfE2wHKnZo7PGvqvNrt6yWG.keystore",
  "password" : "abcd1234"
}
```
...
{
  "keystore" : {
    "address" : "tTOFeBaMujLBcZWfE2wHKnZo7PGvqvNrt6yWG",
...
_**request path:**_
/api/account/export/tTOFeBaMujLBcZWfE2wHKnZo7PGvqvNrt6yWG
...
_**详细描述: 别名格式为1-20位小写字母和数字的组合，设置别名会销毁1个TOF**_
#### HttpMethod: POST
...
```json
{
  "address" : "tTOFeBaMvEtDfvZuukDf2mVyfGo3DdiN8KLRG",
  "alias" : "aaeell1",
  "password" : "TOF123456"
}
```
...
_**request path:**_
/api/accountledger/balance/tTOFeBaMujLBcZWfE2wHKnZo7PGvqvNrt6yWG
...
```json
{
  "success" : true,
  "data" : [ {
    "address" : "tTOFeBaMoS1x2VryPZGyaVSfbaqcLfhqhbXit",
...
```json
{
  "success" : true,
  "data" : {
    "value" : "tTOFeBaNLp2p5hhAapaZz7AYXw6Ysw5t4Ph8M"
  }
}
```
...
```json
{
  "address" : "tTOFeBaMpFkFySUJVXnYKKxye4RYkwRPqQF71",
  "alias" : "tksk11",
  "nonce" : "0000000000000000",
  "remark" : ""
}
...
```json
{
  "address" : "tTOFeBaNLp2p5hhAapaZz7AYXw6Ysw5t4Ph8M",
  "alias" : "taksk2",
  "nonce" : "0000000000000000",
  "remark" : "",
  "pubKeys" : [ "026f5ba56158da0b5ff545c1016ee2a00d9302fbcd6e0e3f3a8cacc8a3a6e19f54", "03245193eaef6f91de9444ffcc48c9944e140337ba1f855a1d6d2a98e3bf048053" ],
...
```json
{
  "success" : true,
  "data" : {
    "value" : "tTOFeBaMvaRhahBAYkZKQFhiSqcC67UiRzoSA"
  }
}
```
...
```json
{
  ...
  "data" : {
    ...
    "size" : 247,
    "packingAddress" : "tTOFeBaMkrt4z9FYEkkR9D6choPVvQr94oYZp",
...
```json
{
  ...
  "data" : {
    ...
    "txs" : [ {
      ...
      "from" : [ ],
      "to" : [ {
        "address" : "tTOFeBaMkrt4z9FYEkkR9D6choPVvQr94oYZp",
        "assetsChainId" : 2,
        "assetsId" : 1,
        "amount" : "100000",
        "lockTime" : 0
...
{
  "address" : "tTOFeBaMvEtDfvZuukDf2mVyfGo3DdiN8KLRG",
  "toAddress" : "tTOFeBaMujLBcZWfE2wHKnZo7PGvqvNrt6yWG",
  "password" : "TOF123456",
  "amount" : 10000000000,
  "remark" : "remark"
}
...
_**详细描述: 根据inputs和outputs离线组装跨链转账交易，用于单账户或多账户的转账交易。交易手续费为inputs里本链主资产金额总和，减去outputs里本链主资产总和，再加上跨链转账的TOF手续费**_
#### HttpMethod: POST
...
```json
{
  "sender" : "tTOFeBaMvEtDfvZuukDf2mVyfGo3DdiN8KLRG",
  "gasLimit" : 20000,
  "price" : 25,
  "password" : "TOF123456",
...
```json
{
  ...
  "data" : {
    ...
    "contractAddress" : "tTOFeBaMx7J2im9edmmyZofHoTWW6nCTbvy3K"
  }
}
```
...
```json
{
  ...
  "methodName" : "transfer",
  "methodDesc" : null,
  "args" : [ "tTOFeBaMnrs6JKrCy6TQdzYJZkMZJDng7QAsD", 990 ]
}
```
...
```json
{
  "fromAddress" : "tTOFeBaMvEtDfvZuukDf2mVyfGo3DdiN8KLRG",
  "password" : "TOF123456",
  "toAddress" : "tTOFeBaMnrs6JKrCy6TQdzYJZkMZJDng7QAsD",
  "contractAddress" : "tTOFeBaNAKfKnLMR5XG5qtwXt5JS1b3QosZxg",
  "amount" : 8000,
  "remark" : "800个"
}
```
...
```json
{
  "fromAddress" : "tTOFeBaMvEtDfvZuukDf2mVyfGo3DdiN8KLRG",
  "password" : "TOF123456",
  "toAddress" : "tTOFeBaMxyMyafiQjq1wCW7cQouyEhRL8njtu",
  "amount" : "400000000",
  "remark" : "向合约转账"
}
```
...
_**request path:**_
/api/contract/balance/token/tTOFeBaNAKfKnLMR5XG5qtwXt5JS1b3QosZxg/tTOFeBaMnrs6JKrCy6TQdzYJZkMZJDng7QAsD
...
_**request path:**_
/api/contract/info/tTOFeBaMxyMyafiQjq1wCW7cQouyEhRL8njtu
...
```json
{
  ...
  "data" : {
    ...
    "address" : "tTOFeBaMxyMyafiQjq1wCW7cQouyEhRL8njtu",
    "creater" : "tTOFeBaMvEtDfvZuukDf2mVyfGo3DdiN8KLRG",
...
```json
{
  ...
  "data" : {
    ...
    "data" : {
      "success" : true,
      "errorMessage" : null,
      "contractAddress" : "tTOFeBaN1rhd9k9eqNkvwC9HXBWLQ79dRuy81",
      ...
      "transfers" : [ {
        ...
        "from" : "tTOFeBaN1rhd9k9eqNkvwC9HXBWLQ79dRuy81",
        "value" : "200000000",
        "outputs" : [ {
          "to" : "tTOFeBaMp9wC9PcWEcfesY7YmWrPfeQzkN1xL",
          "value" : "100000000"
        }, {
          "to" : "tTOFeBaMshNPEnuqiDhMdSA4iNs6LMgjY6tcL",
          "value" : "100000000"
        } ],
      ...
      "multyAssetTransfers" : [ {
        ...
        "from" : "tTOFeBaN31HBrLhXsWDkSz1bjhw5qGBcjafVJ",
        "value" : "200000000",
        "assetChainId" : 5,
        "assetId" : 1,
        "outputs" : [ {
          "to" : "tTOFeBaMrbMRiFAUeeAt6swb4xVBNyi81YL24",
          "value" : "200000000",
          "assetChainId" : 5,
          "assetId" : 1,
          "lockTime" : 0
      ...
      "token721Transfers" : [ {
        "contractAddress" : "TOFd6Hgrsk44itdzFqjgkgAF6nFM82WdpqrQ",
        "from" : "TOFd6Hgd3ACi95QvpLBfp3jgJP3YFmEpbgoG",
        "to" : "TOFd6HgcbwRjN8AxpPK8TvJWtzBzMQ1zDhVd",
        "tokenId" : "13450",
        "name" : "nft",
        "symbol" : "NFT"
      } ],
      "invokeRegisterCmds" : [ {
        ...
        "args" : {
          ...
          "contractAddress" : "tTOFeBaMzZedU4D3xym1JcyNa5sqtuFku8AKm",
          "contractNonce" : "0000000000000000",
          "blockTime" : 1562564381,
          "packingAddress" : "tTOFeBaMtEPLXxUgyfnBt9bpb5Xv84dyJV98p",
          "contractSender" : "tTOFeBaMvEtDfvZuukDf2mVyfGo3DdiN8KLRG"
        },
...
```json
{
  ...
  "data" : {
    "c2460b94430074dd98e497ed9d48afb8f44d1323b73ca2086f5abaa0684b760d" : {
      "success" : true,
      "errorMessage" : null,
      "contractAddress" : "tTOFeBaN5Y2gRias1NMNVmsmXqJbu5Bcp3ZPL",
      "result" : null,
      "gasLimit" : 20000,
      "gasUsed" : 13429,
      "price" : 30,
      ...
      "transfers" : [ ],
      "multyAssetTransfers" : [ ],
      "events" : [ "{\"contractAddress\":\"tTOFeBaN5Y2gRias1NMNVmsmXqJbu5Bcp3ZPL\",\"blockNumber\":68,\"event\":\"TransferEvent\",\"payload\":{\"from\":null,\"to\":\"tTOFeBaMvEtDfvZuukDf2mVyfGo3DdiN8KLRG\",\"value\":\"800000\"}}" ],
      "debugEvents" : [ ],
      "tokenTransfers" : [ {
        "contractAddress" : "tTOFeBaN5Y2gRias1NMNVmsmXqJbu5Bcp3ZPL",
        "from" : null,
        "to" : "tTOFeBaMvEtDfvZuukDf2mVyfGo3DdiN8KLRG",
        "value" : "800000",
        "name" : "io",
        "symbol" : "IO",
        "decimals" : 1
    ...
    "48b2f348f201f9d10848f4031a746919470b679f621327b0e0edf50a339f2e87" : {
      ...
      "transfers" : [ ],
      "multyAssetTransfers" : [ ],
      "events" : [ "{\"contractAddress\":\"tTOFeBaN5Y2gRias1NMNVmsmXqJbu5Bcp3ZPL\",\"blockNumber\":71,\"event\":\"TransferEvent\",\"payload\":{\"from\":\"tTOFeBaMvEtDfvZuukDf2mVyfGo3DdiN8KLRG\",\"to\":\"tTOFeBaMtkzQ1tH8JWBGZDCmRHCmySevE4frM\",\"value\":\"4000\"}}" ],
      "debugEvents" : [ ],
      "tokenTransfers" : [ {
        "contractAddress" : "tTOFeBaN5Y2gRias1NMNVmsmXqJbu5Bcp3ZPL",
        "from" : "tTOFeBaMvEtDfvZuukDf2mVyfGo3DdiN8KLRG",
        "to" : "tTOFeBaMtkzQ1tH8JWBGZDCmRHCmySevE4frM",
        "value" : "4000",
        "name" : "io",
        "symbol" : "IO",
        "decimals" : 1
    ...
    "2e99610b7d295790b636fcdb8acf72d70fcae61c873df0984ef248bbbaa6daa2" : {
      ...
      "transfers" : [ ],
      "multyAssetTransfers" : [ ],
      "events" : [ "{\"contractAddress\":\"tTOFeBaN5Y2gRias1NMNVmsmXqJbu5Bcp3ZPL\",\"blockNumber\":72,\"event\":\"ApprovalEvent\",\"payload\":{\"owner\":\"tTOFeBaMvEtDfvZuukDf2mVyfGo3DdiN8KLRG\",\"spender\":\"tTOFeBaMtkzQ1tH8JWBGZDCmRHCmySevE4frM\",\"value\":\"4000\"}}" ],
      "debugEvents" : [ ],
      "tokenTransfers" : [ ],
      "token721Transfers" : [ ],
      "invokeRegisterCmds" : [ ],
...
```json
{
  "contractAddress" : "tTOFeBaMxyMyafiQjq1wCW7cQouyEhRL8njtu",
...
[ {
  ...
  "success" : true,
  "data" : {
    "msg" : "Data error;contract error - contract[tTOFeBaMx7J2im9edmmyZofHoTWW6nCTbvy3K] has stopped",
    "success" : false,
    "code" : "err_0014"
  }
} ]
...
```json
{
  ...
  "methodDesc" : null,
  "args" : [ "tTOFeBaMnrs6JKrCy6TQdzYJZkMZJDng7QAsD" ]
}
```
...
```json
{
  ...
  "data" : {
    ...
    "contractAddress" : "tTOFeBaMwYiR4p1X9xNJPiyJfrXjr4KgkcFjG",
...
```json
{
  ...
  "methodName" : "transfer",
  "methodDesc" : null,
  "args" : [ "tTOFeBaMnrs6JKrCy6TQdzYJZkMZJDng7QAsD", 3800 ],
...
```json
{
  ...
  "nonce" : "9c0aea02bed90ddd",
  "toAddress" : "tTOFeBaMnrs6JKrCy6TQdzYJZkMZJDng7QAsD",
  "contractAddress" : "tTOFeBaN3MH7HX8kXzKw4X9tLKQ991X1GiAbK",
  "gasLimit" : 14166,
  "amount" : 10,
  "remark" : "1个"
}
...
```json
{
  "agentAddress" : "tTOFeBaMujLBcZWfE2wHKnZo7PGvqvNrt6yWG",
  "packingAddress" : "tTOFeBaMvEtDfvZuukDf2mVyfGo3DdiN8KLRG",
  "rewardAddress" : "tTOFeBaMujLBcZWfE2wHKnZo7PGvqvNrt6yWG",
  "commissionRate" : 10,
  "deposit" : "2000000000000",
  "password" : "abcd1234"
}
...
```json
{
  "agentAddress" : "tTOFeBaNLp2p5hhAapaZz7AYXw6Ysw5t4Ph8M",
  "packingAddress" : "tTOFeBaMpFkFySUJVXnYKKxye4RYkwRPqQF71",
  "rewardAddress" : "tTOFeBaNLp2p5hhAapaZz7AYXw6Ysw5t4Ph8M",
  "commissionRate" : 10,
