...
<script>
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        transferForm: {
          ...
          type: this.$route.query.accountType ? this.$route.query.accountType.account : 'TOF',
          amount: '0.01',
          senior: false,
          gas: this.gasNumber,
    ...
    methods: {
        ...
        if (this.transferForm.toAddress) { //转入地址有值
          ...
          if (toAddress.type === 2) { //向合约地址转账nuls
            ...
            if (fromAddress.chainId === toAddress.chainId) { //不跨链交易
              ...
              for (let item of this.assetsList) {
                //判断nuls资产是否够手续费
                let isNulsFee = item.symbol === "TOF" && Number(item.balance) < 0.01;
                //判断本链资产是否够手续费
       ...
       **/
      changeNuls(type = 1) {
        let defaultType = sessionStorage.hasOwnProperty('info') ? JSON.parse(sessionStorage.getItem('info')).defaultAsset.symbol : 'TOF';
        if (type === 0) {
