  ...
  <div class="bg-gray transfer" v-loading="transferLoading" element-loading-spinner="el-icon-loading">
    ...
    <div class="w1200 transfer_info">
      <el-form :model="transferForm" status-icon :rules="transferRules" ref="transferForm" class=" w630 transfer_form">
        ...
        <div class="font14 mb_20">
          ...
          <span class="fCN">{{defaultInfo.defaultAsset && defaultInfo.defaultAsset.symbol}}</span>
        </div>
    ...
    <el-dialog :title="$t('transfer.transfer6')" width="46rem" class="confirm-dialog"
      ...
      <div class="bg-white">
        ...
        <div class="div-data">
          <p>{{$t('public.fee')}}&nbsp;</p>
          <label>{{transferForm.fee}} <span class="fCN">{{defaultInfo.defaultAsset && defaultInfo.defaultAsset.symbol}}</span></label>
        </div>
  ...
  export default {
    ...
    data() {
      ...
      let validateAmount = async (rule, value, callback) => {
        ...
        if (this.assetsInfo.type === 1 && this.assetsInfo.symbol === 'TOF') {
        ...
        } else if (value < 0.001 && this.assetsInfo.symbol === 'TOF') {
    ...
    methods: {
      ...
      async getCapitalListByAddress(address) {
        ...
        if (this.$route.query.accountType === 'TOF') {
          newInfo = {type: 1, tokenSymbol: MAIN_INFO};
          newInfo.tokenSymbol.symbol = 'TOF';
        }
      ...
      async passSubmit(password) {
        ...
        if (this.toAddressInfo.transferType === 1) { //1:NULS转账
          ...
          try {
            ...
            if (!resData.hasOwnProperty('result')) {
              this.$message({
                message: this.$t('tips.tips00') + JSON.stringify(resData.error),
                type: 'error',
                duration: 3000
              });
              this.transferLoading = false;
            ...
            //console.log(err);
            this.$message({message: this.$t('tips.tips00') + JSON.stringify(err), type: 'error', duration: 3000});
            this.transferLoading = false;
          }
          return;
        ...
        if (!broadcastResult.success) {
          this.$message({
            message: this.$t('tips.tips00') + JSON.stringify(broadcastResult.data),
            type: 'error',
            duration: 3000
          });
          this.transferLoading = false;
      ...
      async crossTxhexs(pri, pub, chainId, transferInfo) {
        ...
        if (chainId === transferInfo.assetsChainId && transferInfo.assetsId === 1) {
          let newAmount = Plus(transferInfo.amount, transferInfo.fee).toString();
