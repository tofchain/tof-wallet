<template>
  <div class="header">
    <div class="w1200">
      <div class="logo">
        <img class="click" @click="toUrl('home')" src="../assets/img/tof-icon.png">
      </div>
      <div class="nav">
        <el-menu mode="horizontal" :default-active="navActives($route.path)" @select="handleSelect">
          ...
          <el-menu-item index="application" v-if="false">{{$t('nav.application')}}</el-menu-item>
        </el-menu>
      </div>
      <div class="tool">
...
<script>
  ...
  export default {
    ...
    mounted() {
      setInterval(() => {
        this.symbol = sessionStorage.hasOwnProperty('info') ? JSON.parse(sessionStorage.getItem('info')).defaultAsset.symbol : 'TOF';
